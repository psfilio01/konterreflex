...
export function createStripeWebhookHandler(dependencies: StripeWebhookDependencies) {
  return async (request: Request): Promise<Response> => {
    ...
    if (event == null) return Response.json({ error: "invalid_payload" }, { status: 400 });
    try {
...
export async function verifyStripeSignature(
  ...
  if (!Number.isFinite(timestamp) || candidates.length === 0 || !secret) return false;
...
function stripeEvent(value: unknown): StripeEventSync | null {
  if (!isRecord(value) || typeof value.id !== "string" || typeof value.type !== "string" || typeof value.created !== "number") return null;
  const result: StripeEventSync = { eventId: value.id, eventType: value.type, eventCreated: value.created };
  if (!value.type.startsWith("customer.subscription.")) return result;
  const data = value.data;
  if (!isRecord(data) || !isRecord(data.object)) return null;
  const object = data.object;
  if (typeof object.id !== "string" || typeof object.status !== "string") return null;
  const metadata = isRecord(object.metadata) ? object.metadata : {};
  const customer = typeof object.customer === "string"
    ? object.customer
    : isRecord(object.customer) && typeof object.customer.id === "string" ? object.customer.id : undefined;
  const period = typeof object.current_period_end === "number"
    ? object.current_period_end
    : firstItemPeriodEnd(object.items);
  return {
    ...result,
    ...(typeof metadata.user_id === "string" && isUuid(metadata.user_id) ? { userId: metadata.user_id } : {}),
    customerId: customer,
    subscriptionId: object.id,
    subscriptionStatus: object.status,
    ...(period == null ? {} : { currentPeriodEnd: new Date(period * 1000).toISOString() }),
  };
}
...
function firstItemPeriodEnd(value: unknown): number | undefined {
  ...
  return typeof value.data[0].current_period_end === "number" ? value.data[0].current_period_end : undefined;
}
